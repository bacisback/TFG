...
o = [s for s in onlfiles if ("HSIC" in s)]
#o = onlfiles
...
#    title = f.replace("permutacionesVSasintHSIC-","").replace(str(size),"").replace("-.txt","")
  ...
  for i,correlacion in enumerate(np.linspace(0,1,5)): 
    solutions = np.loadtxt("./datos/permutacionesVSasint/"+"HSIC-"+str(correlacion)+"-"+str(size)+"version.txt",delimiter="\t")
  ...
  #plt.show()
  plt.savefig("figuras/AsimptoticVsReal/HSIC"+str(size)+".png")
